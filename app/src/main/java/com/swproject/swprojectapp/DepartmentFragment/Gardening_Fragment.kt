...
class Gardening_Fragment : Fragment() {
    ...
    fun searchCrawlingThread(page: Int, value: String = "") {
        //스레드 생성
        thread {
            val URL = "http://swu-hort.swu.ac.kr/bbs/bbs/?bbs_no=6&page_no=${page}&search_kind=subject&search_text=${value}"
            ...
            if (elements != null) {
                ...
                for (element in elements) {
                    ...
                    if((title != "")){
                        val link = "http://swu-hort.swu.ac.kr/bbs/bbs/view.php?bbs_no=6&data_no=" + element.getElementsByTag("a").attr("value")
                        val value = element.getElementsByTag("a").attr("value")
                        val noticeData = NoticeData(title,date, link, "gardening" + value)
