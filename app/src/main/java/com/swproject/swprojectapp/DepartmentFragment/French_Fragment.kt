...
class French_Fragment : Fragment() {
    ...
    fun crawlingThread(page: Int) {
        //스레드 생성
        thread {
            val URL = "http://france.swu.ac.kr/bbs/bbs/?bbs_no=16&page_no=${page}"
            ...
            if (elements != null) {
                ...
                for (element in elements) {
                    ...
                    if((title != "")){
                        val link = "http://france.swu.ac.kr/bbs/bbs/view.php?bbs_no=16&data_no=" + element.getElementsByTag("a").attr("value")+"&page_no=${page}"
                        val id = element.getElementsByTag("a").attr("value")
                        val noticeData = NoticeData(title,date, link, "french" + id)
                        noticeDatas.add(noticeData)
                        //북마크 저장할때 사용할 키

                        //val pushKey = FBRef.bookmarkRef.child(Auth.current_uid).push().key
                        //pushKeyList.add("french" + id)
                    }
