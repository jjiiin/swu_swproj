...
class FiveFragement : Fragment() {
    ...
    fun crawlingThread(page: Int) {
        //스레드 생성
        thread {
            val URL = "https://www.swu.ac.kr/www/noticee.html"//
            ...
            if (elements != null) {
                ...
                for (element in elements) {
                    ...
                    val noticeData = NoticeData(title, date, link, "five" + pkid,top)
                    noticeDatas.add(noticeData)
                    //북마크 저장할때 사용할 키
                    //val pushKey = FBRef.bookmarkRef.child(Auth.current_uid).push().key
                    //pushKeyList.add("five" + pkid)
                }
